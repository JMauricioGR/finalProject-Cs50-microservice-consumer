...
import com.santiagoposadag.cs50.receiverconsumer.usecases.GetAllUserActionsUseCase;
...
public class QueryRouter {
    ...
    @Bean
    public RouterFunction<ServerResponse> getUSerActionAll(GetAllUserActionsUseCase getAllUserActionsUseCase) {
        return route(GET("/getUsersAll"),
                request -> ServerResponse.ok()
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(BodyInserters.fromPublisher(
                                getAllUserActionsUseCase.apply(request.pathVariable("userId")),
                                UserDto.class
                        ))
        );
    }
